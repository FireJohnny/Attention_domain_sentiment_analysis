...
"""
from train_model.att_model import *
...
def parses():
    ...
    tf.flags.DEFINE_list("cnn_filter_size", [2, 3, 4], "convolution filter size(default=[2,3,4])")
...
def train(data, checkpoint_dir=None, gpu_id=0, log_dir="./log"):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            model = ATT_model(emb_dim=FLAGS.emb_size, length=data["sentence_len"],
            ...
            def train(x1_v, x2_v, x1_tf, x2_tf, x1_label, x2_label, step):
                for b_x1_v, b_x2_v, b_x1_tf, b_x2_tf, b_x1_label, b_x2_label in creat_batch(x1_v, x2_v, x1_tf, x2_tf,
                                                                                            x1_label, x2_label,
                                                                                            batch_size=64):
                    ...
                    if step > 19999 and step % 100 == 0:
                        ...
                        for dev_x1, dev_x2, dev_x1_tf, dev_x2_tf, x1_labels, x2_labels, \
                                in creat_batch(data["x1_dev"], data["x2_dev"], data["x1_tf_dev"], data["x2_tf_dev"],
                                               data["x1_dev_labels"], data["x2_dev_labels"], batch_size=64):
                            dev_x1_a, dev_x2_a, d_loss = dev(dev_x1, dev_x2, dev_x1_tf, dev_x2_tf, x1_labels, x2_labels,
                                                             step)
                        ...
                        # dev_summary = tf.Summary(value = [tf.Summary.Value(tag="dev_x1_acc",simplevalue=dev_x1_acc),
                        #                                   tf.Summary.Value(tag="dev_x2_acc",simplevalue=dev_x2_acc),
                        #                                   tf.Summary.Value(tag="dev_loss",simplevalue=dev_loss)])
            ...
            for epoch in range(epoches):
                step = train(data["x1_train"], data["x2_train"], data["x1_tf_tr"], data["x2_tf_tr"], data["x1_labels"],
                             data["x2_labels"], step)
...
if __name__ == '__main__':
    ...
    target = "dvd"
    gpu_id = 0
    breakpoint_train = "False"
    log_dir = "./log{}".format(gpu_id)

    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
        if not os.path.exists(log_dir + "/" + source + "_" + target):
            os.mkdir(log_dir + "/" + source + "_" + target)

    data = data_clean(source, target)
    if breakpoint_train.lower() == "false":
        checkpoint_dir = None
