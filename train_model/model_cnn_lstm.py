...
class model():
    def __init__(self, emb_dim, length, vocab_size, filter_size, conv_out, lstm_cell, sentence_len ):
        ...
        self.s_len = sentence_len

    def _creatplacehoder(self):
        self.x_1_word = tf.placeholder(dtype=tf.int32, shape=[None, self.length]) #source
        self.x_1_tf = tf.placeholder(dtype=tf.int32, shape=[None, self.s_len])
        self.x_2_word = tf.placeholder(dtype=tf.int32, shape=[None, self.length]) #target
        self.x_2_tf = tf.placeholder(dtype=tf.int32, shape=[None, self.s_len])
        self.labels = tf.placeholder(dtype=tf.int32, shape=[None, 2])
        self.d_label = tf.placeholder(dtype=tf.int32,shape=[None, 2])
    ...
    def creat_model(self):
        ...
        self.rnn_x1 = rnn_model(name="rnn_x1",)      #input tensor: [batch, length, embed_size]
        self.rnn_x2 = rnn_model(name="rnn_x2",)
    ...
    def CNN_layer(self,variable_scope, x1, x2):
        with tf.variable_scope(variable_scope) as scope:
            ...
            L_conv = self.cnn_x1(x1, kernel_size=(self.filter_size, self.emb_dim),reuse = False)
